...
def std_format_els(oldelsdict):
    ...
    if not elsdict['name']:
        ...
        # insert assumed global values if absent
        if not elsdict['eph']:
            elsdict['eph'] = 'JPLDE431'
        if not elsdict['refsys']:
            elsdict['refsys'] = 'ECLM J2000'

        # construct names dictionary
        elsdict['names'] = to_names_dict(elsdict['name'])
        del elsdict['name']
        ...
        for coordtype in ['EQU','KEP','CAR','COM','COT']:
            ...
            if coordtype in elsdict.keys() and elsdict[coordtype]:
                ...
                thiscoorddict['nongrav_info'] = translate_nongravs(thiscoorddict)
...
def to_nums(v):
    ...
    if isinstance(v, dict):
        ...
        print('to_nums : did not think of that', type(v))
        return v
